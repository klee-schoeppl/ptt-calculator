...
AC<-1 + a1*c1 # the total number of A and C
CnA<- 1 + c1*(1-a1)# the total number of not-A and C
pCgA<-AC/A
pCgnA<-CnA/nA
deltaP<-pCgA-pCgnA
output<-cbind(AC,A,CnA,nA,pCgA,pCgnA,deltaP)
...
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
# TRYING MY HANDS AT A SUBROUTINE FOR THIS PROBLEM

calculateDeltaP <- function(vecA, vecC) {
  #Expects as input 2 equal length vectors containing 6 or fewer boolean values 
  #each. VecA should contain whether the die in question makes the 
  ...
  a5 <- c(0)
  a6 <- c(0)
  aBlanks = list(a1, a2, a3, a4, a5, a6)
  c1 <- c(0)
  c2 <- c(0)
  c3 <- c(0)
  c4 <- c(0)
  c5 <- c(0)
  c6 <- c(0)
  cBlanks = list(c1, c2, c3, c4, c5, c6)
  #rm(constituents)
  consituents <- list()
  print(length(vecA))
  numberOfConceiledSides <- (6 - length(vecA))
  
  if (numberOfConceiledSides>0){ #preparation to expand the grid
  ...
  print('test')
  print(constituents)
  print(length(constituents))
  if(numberOfConceiledSides>0){ #gridExpansion
    constituents <- rev(expand.grid(constituents))
  }
  ...
  #for blank sides. 
  A <- sum(vecA) #counts all T entries in the vector
  A <- A + sum(aBlanks[[1]],aBlanks[[2]],aBlanks[[3]],aBlanks[[4]],aBlanks[[5]],aBlanks[[6]]) #irrelevant values equal 0
  print(A)
  C <- sum(vecC) #counts all T entries in the vector
  C <- C + sum(cBlanks[[1]],cBlanks[[2]],cBlanks[[3]],cBlanks[[4]],cBlanks[[5]],cBlanks[[6]]) #irrelevant values equal 0
  print(C)
  nA<- 6 - A
  print(nA)
  AC<-0 + aBlanks[[1]]*cBlanks[[1]] + aBlanks[[2]]*cBlanks[[2]] 
  + aBlanks[[3]]*cBlanks[[3]] + aBlanks[[4]]*cBlanks[[4]] + aBlanks[[5]]*cBlanks[[5]] 
  + aBlanks[[6]]*cBlanks[[6]] 
  ...
  print(AC)
  CnA<- 0 + cBlanks[[1]]*(1-aBlanks[[1]]) + cBlanks[[2]]*(1-aBlanks[[2]]) 
  + cBlanks[[3]]*(1-aBlanks[[3]]) + cBlanks[[4]]*(1-aBlanks[[4]]) 
  + cBlanks[[5]]*(1-aBlanks[[5]])+ cBlanks[[6]]*(1-aBlanks[[6]])
  for (i in 1 : length(vecA)){
    if (!vecA[i] & vecC[i]){
      CnA <- CnA + 1
    }
...
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# TESTING GROUND FOR MY SUBROUTINE
vectorA <- c(T,F,F,F,F)
vectorC <- c(T,T,T,F,F)
